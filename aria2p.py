...
class Aria2Client:
    ...
    def __init__(self, host=None, port=None, server=None, secret=None):
        if host is None:
            host = "http://localhost"
        else:
            host = host.rstrip("/")
        ...
        if server is None:
            server = "{host}:{port}/jsonrpc".format(host=host, port=port)
    ...
    # utils
    def call(self, method, params=None, msg_id=None, insert_secret=True, return_json=False):
        ...
        return self.post(self.get_payload(method, params, msg_id=msg_id), return_json)

    def batch_call(self, calls, insert_secret=True, return_json=False):
        payloads = []
        ...
        return self.post(payload, return_json)

    def multicall2(self, calls, insert_secret=True, return_json=False):
        multicall_params = []
    ...
    def post(self, payload, return_json=True):
        response = requests.post(self.server, data=payload)
        return response.text if return_json else response.json()
    ...
    def tell_status(self, gid, keys=None):
        ...
        followedBy
               ...
               option). This value is useful to track auto-generated downloads. If there are no such down‐ loads,
    ...
    def get_files(self, gid):
        ...
        completedLength
               ...
               com‐ pletedLength in aria2.tellStatus() also includes partially completed pieces.

        selected
               true if this file is selected by --select-file option. If --select-file is not specified or this is
               single-file torrent or not a torrent down‐ load at all, this value is always true. Otherwise false.
    ...
    def change_uri(self, gid, file_index, del_uris, add_uris, position=None):
        ...
        remove them all, you have to specify (at least) 3 http://exam‐ ple.org/aria2 in delUris. This method
        ...
        """
        return self.call(
            self.CHANGE_URI, [gid, file_index, del_uris, add_uris, position]
        )
    ...
    def get_global_stat(self):
        ...
        numStopped
                Total The number of stopped downloads in the current session and not capped by the
                --max-download-result option.
    ...
    def force_shutdown(self):
        ...
        actions which take time, such as contacting Bit‐ Torrent trackers to unregister downloads first. This method
        returns OK.
        """
...
class Methods(dict):
    def __init__(self, **kwargs):
        ...
        for method in [
            Aria2Client.ADD_URI,
            Aria2Client.ADD_TORRENT,
            Aria2Client.ADD_METALINK,
            Aria2Client.REMOVE,
            Aria2Client.FORCE_REMOVE,
            Aria2Client.PAUSE,
            Aria2Client.PAUSE_ALL,
            Aria2Client.FORCE_PAUSE,
            Aria2Client.FORCE_PAUSE_ALL,
            Aria2Client.UNPAUSE,
            Aria2Client.UNPAUSE_ALL,
            Aria2Client.TELL_STATUS,
            Aria2Client.GET_URIS,
            Aria2Client.GET_FILES,
            Aria2Client.GET_PEERS,
            Aria2Client.GET_SERVERS,
            Aria2Client.TELL_ACTIVE,
            Aria2Client.TELL_WAITING,
            Aria2Client.TELL_STOPPED,
            Aria2Client.CHANGE_POSITION,
            Aria2Client.CHANGE_URI,
            Aria2Client.GET_OPTION,
            Aria2Client.CHANGE_OPTION,
            Aria2Client.GET_GLOBAL_OPTION,
            Aria2Client.CHANGE_GLOBAL_OPTION,
            Aria2Client.GET_GLOBAL_STAT,
            Aria2Client.PURGE_DOWNLOAD_RESULT,
            Aria2Client.REMOVE_DOWNLOAD_RESULT,
            Aria2Client.GET_VERSION,
            Aria2Client.GET_SESSION_INFO,
            Aria2Client.SHUTDOWN,
            Aria2Client.FORCE_SHUTDOWN,
            Aria2Client.SAVE_SESSION,
            Aria2Client.MULTICALL,
            Aria2Client.LIST_METHODS,
            Aria2Client.LIST_NOTIFICATIONS,
        ]:
...
def get_parser():
    ...
    rpc_group.add_argument("-m", "--rpc-method", dest="method")

    rpc_params_group = rpc_group.add_mutually_exclusive_group()
    rpc_params_group.add_argument("-p", "--rpc-params", dest="params", nargs="+")
    rpc_params_group.add_argument("--json-rpc-params", dest="json_params")

    # general_group = mutually_exclusive.add_argument_group()
    # subcommands: list, add, pause, resume, stop, remove, search, info
    # use click?
...
if __name__ == "__main__":
    ...
    if args.method:
        ...
        if method is None:
            print("unkwown method")
            sys.exit(1)
        print(client.call(method, params, return_json=True))
        sys.exit(0)

    torrents = []

    fields = [
        "gid", "status", "totalLength", "completedLength", "uploadLength", "downloadSpeed", "uploadSpeed", "infoHash",
        "numSeeders", "files"
    ]
    torrents.extend(client.tell_active(keys=fields).get("result", []))
    torrents.extend(client.tell_waiting(0, 1000, keys=fields).get("result", []))
    torrents.extend(client.tell_stopped(0, 1000, keys=fields).get("result", []))

    for torrent in torrents:
        if float(torrent["totalLength"]) == 0.0:
            progress = "?"
        else:
            progress = "%.2f%%" % (float(torrent["completedLength"]) / float(torrent["totalLength"]) * 100)
        print("{gid: <16} {status: <10} {progress: <8} {files}".format(
            gid=torrent["gid"],
            status=torrent["status"],
            progress=progress,
            files=("\n" + " " * (16+10+8+3)).join(t["path"] for t in torrent["files"])
        ))
