...
class Shared:
    ...
    def get_payload(
        ...
        payload: dict[str, Any] = {
            "jsonrpc": "2.0",
            "method": method,
            "id": DEFAULT_ID if msg_id is None else msg_id,
        }
    ...
    async def listen_to_notifications(
        self,
        on_download_start: Union[Callable, None] = None,
        on_download_pause: Union[Callable, None] = None,
        on_download_stop: Union[Callable, None] = None,
        on_download_complete: Union[Callable, None] = None,
        on_download_error: Union[Callable, None] = None,
        on_bt_download_complete: Union[Callable, None] = None,
        timeout: int = 5,
        handle_signals: bool = True,  # noqa: FBT001,FBT002
    ) -> None:
        """Start listening to aria2 notifications via WebSocket asynchronously.

        This method opens a WebSocket connection to the server and waits for notifications (or events) to be received.
        ...
        callbacks = {
            "onDownloadStart": on_download_start,
            "onDownloadPause": on_download_pause,
            "onDownloadStop": on_download_stop,
            "onDownloadComplete": on_download_complete,
            "onDownloadError": on_download_error,
            "onBtDownloadComplete": on_bt_download_complete,
        }

        loop = asyncio.get_event_loop()
        stopped = SignalHandler(["SIGTERM", "SIGINT"], loop) if handle_signals else False
        executor = ThreadPoolExecutor(max_workers=1000)

        async def handle_notification(callback, gid):
            if iscoroutinefunction(callback):
                await callback(gid)
            elif callable(callback):
                await loop.run_in_executor(executor, callback, gid)
        ...
        async with websockets.connect(ws_server) as websocket:
            while not stopped:
                ...
                try:
                    ...
                    logger.error(
                        f"{log_prefix}: connection to server was closed. Is the server running?"
                    )
                    break
                ...
                if callback:
                    logger.debug(f"{log_prefix}: handling notification with gid={notification.gid}")
                    loop.create_task(handle_notification(callback, notification.gid))
                else:
                    logger.debug(
                        f"{log_prefix}: no callback given for type {notification.type}"
                    )
...
class Client(Shared):
    ...
    def __init__(
        ...
        self.session = httpx.Client(
            timeout=self.timeout, transport=httpx.HTTPTransport(retries=self.retries)
        )
        super().__init__()
    ...
    def add_uri(
        ...
        Parameters:
            ...
            options: `options` is a struct and its members are pairs of option name and value.
                See [SyncOptions][aria2p.options.SyncOptions] for more details.
    ...
    def add_torrent(
        ...
        If [`--rpc-save-upload-metadata`][aria2p.options.SyncOptions.rpc_save_upload_metadata] is true,
        the uploaded data is saved as a file named as the hex string of SHA-1 hash of data plus ".torrent"
        in the directory specified by [`--dir`][aria2p.options.SyncOptions.dir] option.
        ...
        or [`--rpc-save-upload-metadata`][aria2p.options.SyncOptions.rpc_save_upload_metadata] is false,
        the downloads added by this method are not saved by [`--save-session`][aria2p.options.SyncOptions.save_session].
    ...
    def tell_status(self, gid: str, keys: list[str] | None = None) -> dict:
        ...
        - `followedBy`: List of GIDs which are generated as the result of this download. For example, when aria2 downloads a
          ...
          (see the [`--follow-metalink`][aria2p.options.SyncOptions.follow_metalink] option).
    ...
    def get_files(self, gid: str) -> dict:
        ...
        - `selected`: true if this file is selected by [`--select-file`][aria2p.options.SyncOptions.select_file] option.
          If [`--select-file`][aria2p.options.SyncOptions.select_file] is not specified
    ...
    def get_global_stat(self) -> dict:
        ...
        Returns:
            ...
            - `numStopped`: The number of stopped downloads in the current session. This value is capped by the
                [`--max-download-result`][aria2p.options.SyncOptions.max_download_result] option.
    ...
    def save_session(self) -> str:
        ...
        by the [`--save-session`][aria2p.options.SyncOptions.save_session] option.
...
class AsyncClient(Shared):
    ...
    async def call(
        ...
        payload: str = self.get_payload(method, params, msg_id=msg_id)  # type: ignore
        return self.res_or_raise((await self.post(payload)))
    ...
    async def multicall2(
        self, calls: Multicalls2Type, insert_secret: bool = True
    ) -> CallReturnType:  # noqa: FBT001,FBT002
        """Call multiple methods in one request.
    ...
    async def post(self, payload: str) -> dict:
        """Send a POST request to the server.
        ...
        Returns:
            The answer from the server, as a Python dictionary.
        """
        return (await self.session.post(self.server, data=payload)).json()
    ...
    async def add_uri(
        ...
        Parameters:
            uris: `uris` is an array of HTTP/FTP/SFTP/BitTorrent URIs (strings) pointing to the same resource.
                ...
                uris must have only one element and it should be BitTorrent Magnet URI.
            options: `options` is a struct and its members are pairs of option name and value.
                See [AsyncOptions][aria2p.options.AsyncOptions] for more details.
            position: If `position` is given, it must be an integer starting from 0.
                The new download will be inserted at `position` in the waiting queue.
                If `position` is omitted or `position` is larger than the current size of the queue,
                the new download is appended to the end of the queue.
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            '{"id":"qwer","jsonrpc":"2.0","result":"0000000000000001"}'
        """
        return await self.call(self.ADD_URI, params=[uris, options, position])  # type: ignore
    ...
    async def add_torrent(
        ...
        If [`--rpc-save-upload-metadata`][aria2p.options.AsyncOptions.rpc_save_upload_metadata] is true,
        the uploaded data is saved as a file named as the hex string of SHA-1 hash of data plus ".torrent"
        in the directory specified by [`--dir`][aria2p.options.AsyncOptions.dir] option.
        E.g. a file name might be 0a3893293e27ac0490424c06de4d09242215f0a6.torrent.
        If a file with the same name already exists, it is overwritten!
        If the file cannot be saved successfully
        or [`--rpc-save-upload-metadata`][aria2p.options.AsyncOptions.rpc_save_upload_metadata] is false,
        the downloads added by this method are not saved by [`--save-session`][aria2p.options.AsyncOptions.save_session].
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            '{"id":"asdf","jsonrpc":"2.0","result":"0000000000000001"}'
        """
        return await self.call(self.ADD_TORRENT, [torrent, uris, options, position])  # type: ignore
    ...
    async def add_metalink(
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            '{"id":"qwer","jsonrpc":"2.0","result":["0000000000000001"]}'
        """
        return await self.call(self.ADD_METALINK, [metalink, options, position])  # type: ignore
    ...
    async def remove(self, gid: str) -> str:
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            '{"id":"qwer","jsonrpc":"2.0","result":"0000000000000001"}'
        """
        return await self.call(self.REMOVE, [gid])  # type: ignore[return-value]
    ...
    async def force_remove(self, gid: str) -> str:
        ...
        Returns:
            The GID of the removed download.
        """
        return await self.call(self.FORCE_REMOVE, [gid])  # type: ignore
    ...
    async def pause(self, gid: str) -> str:
        ...
        Returns:
            The GID of the paused download.
        """
        return await self.call(self.PAUSE, [gid])  # type: ignore
    ...
    async def pause_all(self) -> str:
        ...
        Returns:
            `"OK"`.
        """
        return await self.call(self.PAUSE_ALL)  # type: ignore
    ...
    async def force_pause(self, gid: str) -> str:
        ...
        Returns:
            The GID of the paused download.
        """
        return await self.call(self.FORCE_PAUSE, [gid])  # type: ignore
    ...
    async def force_pause_all(self) -> str:
        ...
        Returns:
            `"OK"`.
        """
        return await self.call(self.FORCE_PAUSE_ALL)  # type: ignore
    ...
    async def unpause(self, gid: str) -> str:
        ...
        Returns:
            The GID of the resumed download.
        """
        return await self.call(self.UNPAUSE, [gid])  # type: ignore
    ...
    async def unpause_all(self) -> str:
        ...
        Returns:
            `"OK"`.
        """
        return await self.call(self.UNPAUSE_ALL)  # type: ignore
    ...
    async def tell_status(self, gid: str, keys: list[str] | None = None) -> dict:
        ...
        - `followedBy`: List of GIDs which are generated as the result of this download. For example, when aria2 downloads a
          Metalink file, it generates downloads described in the Metalink
          (see the [`--follow-metalink`][aria2p.options.AsyncOptions.follow_metalink] option).
          This value is useful to track auto-generated downloads. If there are no such downloads,
          this key will not be included in the response.
        - `following`: The reverse link for followedBy.
          A download included in followedBy has this object's GID in its following value.
            ...
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             u'result': {u'completedLength': u'5701632',
                         ...
                         u'totalLength': u'34896138'}}
        """
        return await self.call(self.TELL_STATUS, [gid, keys])  # type: ignore

    async def get_uris(self, gid: str) -> dict:
        """Return URIs used in a download.
            ...
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             u'result': [{u'status': u'used',
                          u'uri': u'http://example.org/file'}]}
        """
        return await self.call(self.GET_URIS, [gid])  # type: ignore

    async def get_files(self, gid: str) -> dict:
        """Return file list of a download.
        ...
        - `completedLength`: Completed length of this file in bytes.
          ...
          On the other hand, `completedLength` in [`tell_status()`][aria2p.client.Client.tell_status]
          also includes partially completed pieces.
        - `selected`: true if this file is selected by [`--select-file`][aria2p.options.AsyncOptions.select_file] option.
          If [`--select-file`][aria2p.options.AsyncOptions.select_file] is not specified
          or this is single-file torrent or not a torrent download at all, this value is always true. Otherwise false.
        - `uris` Returns a list of URIs for this file.
          The element type is the same struct used in the [`get_uris()`][aria2p.client.Client.get_uris] method.
            ...
            {u'id': u'qwer',
             ...
             u'result': [{u'index': u'1',
                          ...
                          u'uris': [{u'status': u'used',
                                     u'uri': u'http://example.org/file'}]}]}
        """
        return await self.call(self.GET_FILES, [gid])  # type: ignore

    async def get_peers(self, gid: str) -> dict:
        """Return peers list of a download.
            ...
            {u'id': u'qwer',
             ...
             u'result': [{u'amChoking': u'true',
                         ...
                         {u'amChoking': u'false',
                          ...
                          u'uploadSpeed': u'6890'}]}
        """
        return await self.call(self.GET_PEERS, [gid])  # type: ignore

    async def get_servers(self, gid: str) -> dict:
        """Return servers currently connected for a download.
            ...
            {u'id': u'qwer',
             ...
             u'result': [{u'index': u'1',
                          u'servers': [{u'currentUri': u'http://example.org/file',
                                        ...
                                        u'uri': u'http://example.org/file'}]}]}
        """
        return await self.call(self.GET_SERVERS, [gid])  # type: ignore
    ...
    async def tell_active(self, keys: list[str] | None = None) -> list[dict]:
        ...
        Returns:
            An array of the same structs as returned by the [`tell_status()`][aria2p.client.Client.tell_status] method.
        """
        return await self.call(self.TELL_ACTIVE, [keys])  # type: ignore
    ...
    async def tell_waiting(
        ...
        Returns:
            An array of the same structs as returned by [`tell_status()`][aria2p.client.Client.tell_status] method.
        """
        return await self.call(self.TELL_WAITING, [offset, num, keys])  # type: ignore
    ...
    async def tell_stopped(
        ...
        Returns:
            An array of the same structs as returned by the [`tell_status()`][aria2p.client.Client.tell_status] method.
        """
        return await self.call(self.TELL_STOPPED, [offset, num, keys])  # type: ignore
    ...
    async def change_position(self, gid: str, pos: int, how: str) -> int:
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer', u'jsonrpc': u'2.0', u'result': 0}
        """
        return await self.call(self.CHANGE_POSITION, [gid, pos, how])  # type: ignore
    ...
    async def change_uri(
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer', u'jsonrpc': u'2.0', u'result': [0, 1]}
        """
        return await self.call(self.CHANGE_URI, [gid, file_index, del_uris, add_uris, position])  # type: ignore
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             ...
        """
        return await self.call(self.GET_OPTION, [gid])  # type: ignore
    ...
    async def change_option(self, gid: str, options: dict) -> str:
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer', u'jsonrpc': u'2.0', u'result': u'OK'}
        """
        return await self.call(self.CHANGE_OPTION, [gid, options])  # type: ignore
    ...
    async def get_global_option(self) -> dict:
        ...
        Returns:
            ...
            Values are strings.
        """
        return await self.call(self.GET_GLOBAL_OPTION)  # type: ignore
    ...
    async def change_global_option(self, options: dict) -> str:
        ...
        Returns:
            `"OK"` for success.
        """
        return await self.call(self.CHANGE_GLOBAL_OPTION, [options])  # type: ignore
    ...
    async def get_global_stat(self) -> dict:
        ...
        Returns:
            ...
            - `numWaiting`: The number of waiting downloads.
            - `numStopped`: The number of stopped downloads in the current session. This value is capped by the
                [`--max-download-result`][aria2p.options.AsyncOptions.max_download_result] option.
            - `numStoppedTotal`: The number of stopped downloads in the current session and not capped by the
                [`--max-download-result`][aria2p.options.AsyncOptions.max_download_result] option.
            ...
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             u'result': {u'downloadSpeed': u'21846',
                         ...
                         u'uploadSpeed': u'0'}}
        """
        return await self.call(self.GET_GLOBAL_STAT)  # type: ignore
    ...
    async def purge_download_result(self) -> str:
        ...
        Returns:
            `"OK"`.
        """
        return await self.call(self.PURGE_DOWNLOAD_RESULT)  # type: ignore
    ...
    async def remove_download_result(self, gid: str) -> str:
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer', u'jsonrpc': u'2.0', u'result': u'OK'}
        """
        return await self.call(self.REMOVE_DOWNLOAD_RESULT, [gid])  # type: ignore
            ...
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             u'result': {u'enabledFeatures': [u'Async DNS',
                         ...
                         u'version': u'1.11.0'}}
        """
        return await self.call(self.GET_VERSION)  # type: ignore
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             u'result': {u'sessionId': u'cd6a3bc6a1de28eb5bfa181e5f6b916d44af31a9'}}
        """
        return await self.call(self.GET_SESSION_INFO)  # type: ignore
    ...
    async def shutdown(self) -> str:
        ...
        Returns:
            `"OK"`.
        """
        return await self.call(self.SHUTDOWN)  # type: ignore
    ...
    async def force_shutdown(self) -> str:
        ...
        Returns:
            `"OK"`.
        """
        return await self.call(self.FORCE_SHUTDOWN)  # type: ignore

    async def save_session(self) -> str:
        """Save the current session to a file.

        This method saves the current session to a file specified
        by the [`--save-session`][aria2p.options.AsyncOptions.save_session] option.
        ...
        Returns:
            `"OK"` if it succeeds.
        """
        return await self.call(self.SAVE_SESSION)  # type: ignore

    # system
    async def multicall(self, methods: list[dict]) -> list[CallReturnType]:
        """Call multiple methods in a single request.
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            [{u'id': u'qwer', u'jsonrpc': u'2.0', u'result': u'0000000000000001'},
             {u'id': u'asdf', u'jsonrpc': u'2.0', u'result': u'd2703803b52216d1'}]
        """
        return await self.call(self.MULTICALL, [methods])  # type: ignore
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             ...
        """
        return await self.call(self.LIST_METHODS)  # type: ignore

    async def list_notifications(self) -> list[str]:
        """Return all the available RPC notifications.
        ...
        Examples:
            **Original JSON-RPC Example**
            ...
            {u'id': u'qwer',
             ...
             ...
        """
        return await self.call(self.LIST_NOTIFICATIONS)  # type: ignore
