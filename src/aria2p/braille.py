...
class BrailleStream:
    ...
    def reset_height(self, height: int):
        ...
        g = [
            # 0 -> 2 * k + 1
            [[self.lookup[i0][i1] for i0, i1 in zip(blocks[2 * k], blocks[2 * k + 1])] for k in range(self.width)],
            # 1 -> 2 * k + 2
            [[self.lookup[i0][i1] for i0, i1 in zip(blocks[2 * k + 1], blocks[2 * k + 2])] for k in range(self.width)],
        ]
        if not self.flipud:
            g = [
