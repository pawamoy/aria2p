...
def test_add_metalink_method(server):
    assert server.api.add_metalink(DEBIAN_METALINK)


def test_add_torrent_method(server):
    assert server.api.add_torrent(BUNSENLABS_TORRENT)


def test_add_uris_method(server):
    assert server.api.add_uris(XUBUNTU_MIRRORS)


def test_get_download_method(tmp_path, port):
...
def test_get_downloads_method(tmp_path, port):
...
def test_get_global_options_method(tmp_path, port):
...
def test_get_options_method(tmp_path, port):
...
def test_get_stats_method(server):
    assert server.api.get_stats()


def test_move_method(tmp_path, port):
...
def test_move_down_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_move_to_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_move_to_bottom_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_move_to_top_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_move_up_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_pause_method(tmp_path, port):
...
def test_pause_all_method(tmp_path, port):
...
def test_autopurge_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="3-dls.txt") as server:
        assert server.api.autopurge()


def test_remove_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="3-dls.txt") as server:
        downloads = server.api.get_downloads()
...
def test_remove_files_method(tmp_path, port):
...
def test_remove_files_not_complete(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_remove_files_tree(server):
...
def test_remove_all_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="3-dls.txt") as server:
...
def test_resume_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
        downloads = server.api.get_downloads()
...
def test_set_options_method(tmp_path, port):
    with Aria2Server(tmp_path, port, session="max-dl-limit-10000.txt") as server:
        downloads = server.api.get_downloads()
        try:
...
def test_copy_files_method(tmp_path_factory, port):
...
def test_move_files_method(tmp_path_factory, port):
...
def test_listen_to_notifications(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
...
def test_listen_to_notifications_no_thread(tmp_path, port):
    with Aria2Server(tmp_path, port, session="2-dls-paused.txt") as server:
