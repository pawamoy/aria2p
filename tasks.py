...
PY_SRC_PATHS = (Path(_) for _ in ("src", "scripts/*.py", "tests", "tasks.py"))
...
def bundle(context):
    ...
    """
    context.run(
        "poetry run bash -c 'pyinstaller -F -n aria2p -p $VIRTUAL_ENV/lib/python*/site-packages src/aria2p/__main__.py'"
    )
